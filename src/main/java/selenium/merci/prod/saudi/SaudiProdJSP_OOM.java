...
public class SaudiProdJSP_OOM extends SeleniumSEPTestAdvanced{
	...
	public void scenario() throws Exception{
		...
		languageDropdown.selectByVisibleText("Arabic");
...
//		retAvailPage();
	...
	private void checkALPIPage() {

		String firstName = driver.findElement(By.id(PAX_FIRSTNAME)).getText();
		
		String lastName = driver.findElement(By.id(PAX_LASTNAME)).getText();
		
		if(firstName.isEmpty() && lastName.isEmpty()){
			System.out.println("OOM is handled");
		}
	...
	private void enterPAXDetails() {
		...
		driver.findElement(By.id(PAX_FIRSTNAME)).sendKeys("الاسم الأول");
		
		// enter last name
		//TODO: remove hard coding
		driver.findElement(By.id(PAX_LASTNAME)).sendKeys("اسم العائلة");
		
		
		Select countryDropdown = new Select(driver.findElement(By.id("NATIONALITY_1")));

		countryDropdown.selectByVisibleText("الهند");
